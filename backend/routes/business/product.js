        ...
        if(error) {
            ...
            Product.findById(req.params.product_id, function(error, product) {
                if (error) {
                    ...
                    res.render("business/settings/edit", {business_id: req.params.id, product: product, shop: shop});
                }
            })
        } 
    });
...
router.post("/business/:id/shop/:shop_id/product/:product_id", function(req, res) {
    Product.findByIdAndUpdate(req.params.product_id, req.body.product,function(error, product) {
        if(error) {
            console.log(error);
        } else {
...
router.delete("/business/:id/shop/:shop_id/product/:product_id", function(req, res) {
